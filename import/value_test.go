...
func Test_importValue(t *testing.T) {
	...
	for encodedValue, expectedDecodedValue := range expectedResults {
		...
		value := &importValue{ctx: ctx}

		if err := value.UnmarshalJSON([]byte(encodedValue)); err != nil {
			t.Errorf("Unexpected error %s decoding valueerty %s", err, encodedValue)
		}

		if !reflect.DeepEqual(value.Property, expectedDecodedValue) {
			t.Errorf("Expected %s to decode to %+v, but decoded to %+v instead", encodedValue, expectedDecodedValue, value.Property)
		}
